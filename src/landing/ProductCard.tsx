...
/**
 * Cartão de produto exclusivo da LANDING (rota /lp). Diferente da home, exibe o
 * botão "Falar no WhatsApp" (com a mensagem do produto) como ação principal e o
 * CartStepper em navy como secundária. Editar aqui não afeta a home (/) nem o
 * catálogo (/produtos), que têm suas próprias versões. Usa o CartStepper
 * compartilhado (sistema de carrinho) e os utilitários de dados.
 */
export function ProductCard({
  ...
  /** Quando informado, exibe o botão "Falar no WhatsApp" já com a mensagem do produto. */
  whatsappUrl?: string;
}) {
          ...
          {whatsappUrl ? (
            <div className="mt-4 space-y-2">
              <a
                ...
                className="flex h-11 w-full items-center justify-center gap-2 rounded-full bg-red text-sm font-semibold text-white transition hover:bg-red-bright"
              >
                <MessageCircle size={18} aria-hidden="true" /> Falar no WhatsApp
              </a>
              <div className="flex items-center gap-2">
                <CartStepper product={product} variant="navy" className="flex-1" />
                <Link
                  href={`/produtos/${product.slug}`}
